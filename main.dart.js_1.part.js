self.$__dart_deferred_initializers__=self.$__dart_deferred_initializers__||Object.create(null)
$__dart_deferred_initializers__.current=function(a,b,c,$){var A={
aPX(d){return $.aPY.bo(0,d.a.b,new A.a5U(d))},
rN:function rN(d,e,f,g){var _=this
_.e=null
_.f=d
_.b=e
_.c=f
_.a=g},
a5U:function a5U(d){this.a=d},
a5V:function a5V(d){this.a=d},
a5W:function a5W(){},
jj:function jj(d){this.a=d},
Kb:function Kb(d,e){this.a=d
this.b=e},
a5n:function a5n(d,e,f,g){var _=this
_.a=d
_.b=e
_.c=f
_.d=g},
a5m:function a5m(d,e){this.a=d
this.b=e},
a5o:function a5o(d,e,f){this.a=d
this.b=e
this.c=f}},B,C,D,E={a5L:function a5L(d,e){this.a=d
this.b=e}},F,G,H,J,K={
a6d(){var x=0,w=P.R(y.c),v,u,t
var $async$a6d=P.N(function(d,e){if(d===1)return P.O(e,w)
while(true)switch(x){case 0:u=$.hd
x=3
return P.a7((u==null?$.hd=$.qR():u).mH(null,null),$async$a6d)
case 3:t=e
E.eQ(t,$.kL())
v=new K.i2(t)
x=1
break
case 1:return P.P(v,w)}})
return P.Q($async$a6d,w)}},L,M,N={a69:function a69(){}},O,P,Q={
aR7(d){var x=new Q.pd(d,$.a_u())
x.a_8(d)
return x},
pd:function pd(d,e){this.e=null
this.b=d
this.a=e},
abF:function abF(d,e){this.a=d
this.b=e},
abE:function abE(d,e){this.a=d
this.b=e},
abG:function abG(d,e){this.a=d
this.b=e},
abD:function abD(d,e){this.a=d
this.b=e},
abH:function abH(d){this.a=d},
abI:function abI(){},
hR:function hR(d,e){this.a=d
this.$ti=e}},R,S,T,U,V,W,X={
aPZ(){var x=new X.rO(F.cZ(new Z.d2("FirebaseIntegration")))
x.Zs()
return x},
rO:function rO(d){this.a=d},
a65:function a65(d){this.a=d},
a62:function a62(d){this.a=d},
a63:function a63(){},
a64:function a64(){},
a61:function a61(){},
a60:function a60(){}},Y={tq:function tq(d,e){this.c=d
this.a=e}},Z
a.setFunctionNamesIfNecessary([A,E,K,N,Q,X,Y])
A=a.updateHolder(c[0],A)
B=c[1]
C=c[2]
D=c[3]
E=a.updateHolder(c[4],E)
F=c[5]
G=c[6]
H=c[7]
J=c[8]
K=a.updateHolder(c[9],K)
L=c[10]
M=c[11]
N=a.updateHolder(c[12],N)
O=c[13]
P=c[14]
Q=a.updateHolder(c[15],Q)
R=c[16]
S=c[17]
T=c[18]
U=c[19]
V=c[20]
W=c[21]
X=a.updateHolder(c[22],X)
Y=a.updateHolder(c[23],Y)
Z=c[24]
E.a5L.prototype={
rJ(d,e){return this.agQ(0,e)},
agQ(d,e){var x=0,w=P.R(y.v),v=this
var $async$rJ=P.N(function(f,g){if(f===1)return P.O(g,w)
while(true)switch(x){case 0:if(C.c.u(C.T2,e))throw H.b(P.eg(e,"name","Event name is reserved and cannot be used"))
if(C.b.b6(e,"firebase_"))throw H.b(P.eg(e,"name",'Prefix "firebase_" is reserved and cannot be used.'))
x=2
return P.a7(v.a.x_(0,e,null),$async$rJ)
case 2:return P.P(null,w)}})
return P.Q($async$rJ,w)},
tN(d,e){return this.TH(0,e)},
TH(d,e){var x=0,w=P.R(y.v),v=this
var $async$tN=P.N(function(f,g){if(f===1)return P.O(g,w)
while(true)switch(x){case 0:x=2
return P.a7(v.a.tO(0,"Flutter",e),$async$tN)
case 2:return P.P(null,w)}})
return P.Q($async$tN,w)}}
A.rN.prototype={
a8r(d){var x=d.$ti.i("js<bX.T,jj?>")
return P.aTq(new P.js(new A.a5V(this),d,x),null,new A.a5W(),x.i("bX.T"))},
j(d){return"FirebaseAuth(app: "+this.f.a.b+")"}}
A.jj.prototype={
j(d){var x="metadata",w=this.a,v=w.c
return C.a7W.j(0)+"(displayName: "+H.c(v.h(0,"displayName"))+", email: "+H.c(v.h(0,"email"))+", emailVerified: "+H.c(v.h(0,"emailVerified"))+", isAnonymous: "+H.c(v.h(0,"isAnonymous"))+", metadata: "+new Y.alc(J.ar(v.h(0,x),"creationTime"),J.ar(v.h(0,x),"lastSignInTime")).j(0)+", phoneNumber: "+H.c(v.h(0,"phoneNumber"))+", photoURL: "+H.c(v.h(0,"photoURL"))+", providerData, "+H.c(w.gt7(w))+", refreshToken: "+H.c(v.h(0,"refreshToken"))+", tenantId: "+H.c(v.h(0,"tenantId"))+", uid: "+v.h(0,"uid")+")"}}
Q.pd.prototype={
a_8(d){var x=null,w=d.a.b,v=y.w,u=y.F
C.zh.dI("Auth#registerIdTokenListener",P.aB(["appName",w],v,v),!1,v).aH(0,new Q.abF(this,d),u)
C.zh.dI("Auth#registerAuthStateListener",P.aB(["appName",w],v,v),!1,v).aH(0,new Q.abG(this,d),u)
v=y.t
$.aDg.n(0,w,new P.hM(x,x,v))
$.aDh.n(0,w,new P.hM(x,x,v))
$.axi.n(0,w,new P.hM(x,x,v))},
Ak(d,e){return this.a43(d,e)},
a43(d,e){var x=0,w=P.R(y.v),v,u,t,s
var $async$Ak=P.N(function(f,g){if(f===1)return P.O(g,w)
while(true)switch(x){case 0:s=$.aDg.h(0,d)
s.toString
v=$.axh.h(0,d)
v.toString
u=J.ar(e,"user")
if(u==null){v.e=null
s.F(0,C.lF)}else{t=new Y.tq(J.a_L(u,y.w,y.b),$.Fu())
v.e=t
s.F(0,new Q.hR(t,y.e))}return P.P(null,w)}})
return P.Q($async$Ak,w)},
As(d,e){return this.a4J(d,e)},
a4J(d,e){var x=0,w=P.R(y.v),v,u,t,s,r
var $async$As=P.N(function(f,g){if(f===1)return P.O(g,w)
while(true)switch(x){case 0:r=$.aDh.h(0,d)
r.toString
v=$.axi.h(0,d)
v.toString
u=$.axh.h(0,d)
u.toString
t=J.ar(e,"user")
if(t==null){u.e=null
r.F(0,C.lF)
v.F(0,C.lF)}else{s=u.e=new Y.tq(J.a_L(t,y.w,y.b),$.Fu())
u=y.e
r.F(0,new Q.hR(s,u))
v.F(0,new Q.hR(s,u))}return P.P(null,w)}})
return P.Q($async$As,w)},
OO(d){return $.axh.bo(0,d.a.b,new Q.abH(d))},
Gd(d,e){if(d!=null)this.e=new Y.tq(d,$.Fu())
return this},
iw(){var $async$iw=P.N(function(d,e){switch(d){case 2:s=v
x=s.pop()
break
case 1:t=e
x=u}while(true)switch(x){case 0:x=3
v=[1]
return P.h0(P.aFa(r.e),$async$iw,w)
case 3:q=$.axi.h(0,r.gNN(r).a.b)
q.toString
p=H.t(q).i("hN<1>")
x=4
v=[1]
return P.h0(P.D0(new P.js(new Q.abI(),new P.hN(q,p),p.i("js<bX.T,d7?>"))),$async$iw,w)
case 4:case 1:return P.h0(null,0,w)
case 2:return P.h0(t,1,w)}})
var x=0,w=P.ayT($async$iw,y.k),v,u=2,t,s=[],r=this,q,p
return P.ayW(w)}}
Q.hR.prototype={}
Y.tq.prototype={}
N.a69.prototype={
gaid(){var x=this.b
if($.a6a.h(0,x)!=null&&J.ar($.a6a.h(0,x),this.c)!=null)return J.ar($.a6a.h(0,x),this.c)
x=y.b
return P.r(x,x)}}
X.rO.prototype={
wN(d){var x=0,w=P.R(y.b),v=this
var $async$wN=P.N(function(e,f){if(e===1)return P.O(f,w)
while(true)switch(x){case 0:if($.L==null)N.ayc()
$.L.toString
D.aER(new X.a65(v))
x=2
return P.a7(K.a6d(),$async$wN)
case 2:return P.P(null,w)}})
return P.Q($async$wN,w)},
Zs(){this.wN(0).xy(0,new X.a61())}}
A.Kb.prototype={
gl3(){var x=$.j9
return x.guk()},
Rx(){var x=this,w=new A.fe(x.a,x.b,null),v=H.bd("controller")
v.b=new P.hM(new A.a5n(x,v,w,null),new A.a5o(x,w,null),y.C)
return J.aMV(v.aT())},
gaG(d){return this.a}}
var z=a.updateTypes(["rN()","jj?(d7?)","~(eu<jj?>)","pd()","d7?(hR<d7>)","~(jj?)"])
A.a5U.prototype={
$0(){var x=this.a
return new A.rN(x,x.a.b,"plugins.flutter.io/firebase_auth",$.aIa())},
$S:z+0}
A.a5V.prototype={
$1(d){if(d==null)return null
E.eQ(d,$.Fu())
return new A.jj(d)},
$S:z+1}
A.a5W.prototype={
$1(d){return d.aU(0)},
$S:z+2}
Q.abF.prototype={
$1(d){d.toString
new A.Kb(d,C.aT).Rx().mM(0,new Q.abE(this.a,this.b))},
$S:164}
Q.abE.prototype={
$1(d){this.a.As(this.b.a.b,d)},
$S:13}
Q.abG.prototype={
$1(d){d.toString
new A.Kb(d,C.aT).Rx().mM(0,new Q.abD(this.a,this.b))},
$S:164}
Q.abD.prototype={
$1(d){this.a.Ak(this.b.a.b,d)},
$S:13}
Q.abH.prototype={
$0(){return Q.aR7(this.a)},
$S:z+3}
Q.abI.prototype={
$1(d){return d.a},
$S:z+4}
X.a65.prototype={
$1(d){var x
this.a.a.af(C.y,"RoutePathListeners run.",null,null)
x=$.azJ()
x.tN(0,d.c).aH(0,new X.a62(d),y.v).l5(new X.a63(),new X.a64())
x.rJ(0,"page_view")},
$S:137}
X.a62.prototype={
$1(d){return P.a_l("firebaseAnalytics.setCurrentScreen(screenName: "+this.a.c+")","firebaseAnalytics.setCurrentScreen")},
$S:59}
X.a63.prototype={
$1(d){D.ava().$1(": "+H.c(d))},
$S:481}
X.a64.prototype={
$1(d){return d instanceof F.pu},
$S:90}
X.a61.prototype={
$1(d){var x,w,v,u,t,s="APP_CURRENT_USER"
$.azJ().rJ(0,"app_open")
x=$.aI9()
w=x.e
if(w==null){w=x.f
v=x.gaid()
u=$.awU
w=(u==null?$.awU=new Q.pd(null,$.a_u()):u).OO(w)
u=J.am(v)
t=u.h(v,"APP_LANGUAGE_CODE")
w=x.e=w.Gd(u.h(v,s)==null?null:P.te(u.h(v,s),y.w,y.b),t)}x.a8r(w.iw()).mM(0,new X.a60())},
$S:4}
X.a60.prototype={
$1(d){if(d==null)P.a_l("User is currently signed out!","")
else P.a_l("User is signed in!","")},
$S:z+5}
A.a5n.prototype={
$0(){var x=0,w=P.R(y.v),v=1,u,t=[],s=this,r,q,p,o,n,m
var $async$$0=P.N(function(d,e){if(d===1){u=e
x=v}while(true)switch(x){case 0:o=s.a
n=o.a
o.gl3().ps(n,new A.a5m(o,s.b))
v=3
x=6
return P.a7(s.c.dI("listen",s.d,!1,y.v),$async$$0)
case 6:v=1
x=5
break
case 3:v=2
m=u
r=H.a_(m)
q=H.aD(m)
o=U.bs("while activating platform stream on channel "+n)
U.dF(new U.bL(r,q,"services library",o,null,!1))
x=5
break
case 2:x=1
break
case 5:return P.P(null,w)
case 1:return P.O(u,w)}})
return P.Q($async$$0,w)},
$S:40}
A.a5m.prototype={
$1(d){return this.SB(d)},
SB(d){var x=0,w=P.R(y.F),v,u=[],t=this,s,r,q
var $async$$1=P.N(function(e,f){if(e===1)return P.O(f,w)
while(true)switch(x){case 0:if(d==null)J.avO(t.b.aT())
else try{J.h5(t.b.aT(),t.a.b.CV(d))}catch(p){q=H.a_(p)
if(q instanceof F.pu){s=q
t.b.aT().o4(s)}else throw p}v=null
x=1
break
case 1:return P.P(v,w)}})
return P.Q($async$$1,w)},
$S:482}
A.a5o.prototype={
$0(){var x=0,w=P.R(y.v),v=1,u,t=[],s=this,r,q,p,o,n,m
var $async$$0=P.N(function(d,e){if(d===1){u=e
x=v}while(true)switch(x){case 0:o=s.a
n=o.a
o.gl3().ps(n,null)
v=3
x=6
return P.a7(s.b.dI("cancel",s.c,!1,y.v),$async$$0)
case 6:v=1
x=5
break
case 3:v=2
m=u
r=H.a_(m)
q=H.aD(m)
o=U.bs("while de-activating platform stream on channel "+n)
U.dF(new U.bL(r,q,"services library",o,null,!1))
x=5
break
case 2:x=1
break
case 5:return P.P(null,w)
case 1:return P.O(u,w)}})
return P.Q($async$$0,w)},
$S:40};(function inheritance(){var x=a.inheritMany,w=a.inherit
x(P.v,[E.a5L,A.jj,Q.hR,X.rO,A.Kb])
w(N.a69,E.OU)
w(A.rN,N.a69)
x(H.eF,[A.a5U,Q.abH,A.a5n,A.a5o])
x(H.bI,[A.a5V,A.a5W,Q.abF,Q.abE,Q.abG,Q.abD,Q.abI,X.a65,X.a62,X.a63,X.a64,X.a61,X.a60,A.a5m])
w(Q.pd,L.Ko)
w(Y.tq,B.d7)})()
H.da(b.typeUniverse,JSON.parse('{"tq":{"d7":[]}}'))
var y={c:H.z("i2"),F:H.z("ah"),w:H.z("f"),t:H.z("hM<hR<d7>>"),C:H.z("hM<@>"),e:H.z("hR<d7>"),b:H.z("@"),k:H.z("d7?"),v:H.z("~")};(function constants(){var x=a.makeConstList
C.T2=H.a(x(["ad_activeview","ad_click","ad_exposure","ad_impression","ad_query","adunit_exposure","app_clear_data","app_uninstall","app_update","error","first_open","first_visit","in_app_purchase","notification_dismiss","notification_foreground","notification_open","notification_receive","os_update","session_start","user_engagement"]),H.z("p<f>"))
C.zh=new A.fe("plugins.flutter.io/firebase_auth",C.aT,null)
C.a7W=H.al("jj")
C.lF=new Q.hR(null,y.e)})();(function staticFields(){$.aPY=P.r(y.w,H.z("rN"))
$.axh=P.r(y.w,H.z("pd"))
$.aDg=P.r(y.w,H.z("hD<hR<d7>>"))
$.aDh=P.r(y.w,H.z("hD<hR<d7>>"))
$.axi=P.r(y.w,H.z("hD<hR<d7>>"))})();(function lazyInitializers(){var x=a.lazyFinal
x($,"aZI","aIa",function(){return new P.v()})
x($,"aZG","azJ",function(){var w=$.aI6()
U.hT()
return new E.a5L(w,null)})
x($,"aZF","aI9",function(){return A.aPX(K.aQ0())})})()}
$__dart_deferred_initializers__["oOnx+6OewEoaVTfxe4vOHQANvJg="] = $__dart_deferred_initializers__.current
